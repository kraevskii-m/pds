...
class DigitalOceanAPIClient(APIClient):
    ...
    def get_auth_config(self) -> Dict[str, str]:
        return {
            "DIGITALOCEAN_TOKEN": "Personal Access Token from DO dashboard"
        }
    ...
    async def fetch_resources(self) -> ProviderResources:
        """Getting data from DigitalOcean API."""
    ...
    async def _fetch_regions(self) -> List[str]:
        ...
        available_regions = [
            ...
            if r["available"] and "features" in r
        ]
    ...
    async def _fetch_sizes(self) -> List[Dict[str, Any]]:
        ...
        available_sizes = [
            s for s in sizes 
            if s["available"] and s["regions"]
        ]
        
        return available_sizes
    
    def _group_sizes_by_region(self, 
                              regions: List[str], 
                              sizes: List[Dict[str, Any]]) -> Dict[str, List[str]]:
        ...
        all_sizes = list(set(s["slug"] for s in sizes))
    ...
    async def _fetch_database_options(self) -> Dict[str, List[str]]:
        ...
        try:
            response = await self._request_with_retry(
                "GET",
                f"{self.BASE_URL}/databases/options",
                headers=self._get_headers()
            )
    ...
    def get_static_fallback(self) -> ProviderResources:
        ...
        instance_types = {region: static_sizes for region in static_regions}
