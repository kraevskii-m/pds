...
class DynamicSchemaGenerator:
    ...
    async def _get_provider_resources(self, 
                                    client: APIClient,
                                    use_cache: bool = True,
                                    force_refresh: bool = False) -> ProviderResources:
        ...
        try:
            async with client:
                ...
                # Cache the result
                await self.cache.cache_resources(
            ...
            print(f"Failed to fetch {client.provider_name} resources, using fallback: {e}")
            return client.get_static_fallback()
    
    def _build_provider_schema(self, provider_name: str, resources: ProviderResources) -> Dict:
            ...
            for db_type, versions in resources.database_types.items():
                if versions:
                    definitions[f"{provider_title}{db_type.title()}Versions"] = {
                        "type": "string",
                        "enum": versions,
                        "description": f"Available {db_type} versions on {provider_name}"
                    }
        ...
        return {
            ...
            "conditional": self._build_conditional_schema(provider_name, provider_title, resources)
        }
    
    def _build_conditional_schema(self, 
                                 provider_name: str, 
                                 provider_title: str,
                                 resources: ProviderResources) -> Dict:
        ...
        if resources.regions:
            properties["region"] = {
                "$ref": f"#/definitions/{provider_title}Regions"
            }
        ...
        if resources.database_types:
                ...
                if resources.database_types[db_type]:
                    database_props["version"] = {
                        "anyOf": [
                            {
                                "if": {
                                    "properties": {"type": {"const": db_type}}
                                },
                                "then": {
                                    "$ref": f"#/definitions/{provider_title}{db_type.title()}Versions"
                                }
                            }
                        ]
                    }
            ...
            if database_props:
                infrastructure_props["database"] = {
                    "properties": database_props
                }
        
        if infrastructure_props:
            properties["infrastructure"] = {
                "properties": infrastructure_props
            }
        
        return {
            "if": {
                "properties": {"provider": {"const": provider_name}}
            },
            "then": {
    ...
    def _merge_schemas(self, base_schema: Dict, provider_schemas: Dict[str, Dict]) -> Dict:
