...
def generate_schema(
    output: str = typer.Option("pds-schema.json", "--output", "-o", help="Output file path"),
    providers: Optional[List[str]] = typer.Option(None, "--provider", "-p", help="Specific providers to include"),
    live: bool = typer.Option(False, "--live", help="Fetch live data from provider APIs"),
    no_cache: bool = typer.Option(False, "--no-cache", help="Skip cache and force API calls"),
        ...
        try:
            with Progress(
                ...
                if live or force_refresh:
                    task = progress.add_task("Fetching live data from APIs...", total=None)
                else:
            ...
            console.print(f"✅ Schema saved to: [bold green]{output_path.absolute()}[/]")
...
def refresh_cache(
    provider: Optional[str] = typer.Argument(None, help="Provider to refresh (all if not specified)")
):
...
def install_schema(
    editor: str = typer.Option("vscode", "--editor", "-e", help="Editor to install for (vscode, nvim)"),
    workspace: str = typer.Option(".", "--workspace", "-w", help="Workspace directory"),
    schema_file: str = typer.Option("pds-schema.json", "--schema", "-s", help="Schema file path")
):
...
def validate_config(
    config_file: str = typer.Argument("pds.yaml", help="Configuration file to validate")
):
